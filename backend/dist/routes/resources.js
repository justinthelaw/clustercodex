...
import { AppsV1Api, BatchV1Api, CoreV1Api, KubeConfig, NetworkingV1Api, ApiextensionsV1Api } from "@kubernetes/client-node";
...
function formatPodStatus(pod) {
    const status = pod.status?.phase || "Unknown";
    const waiting = pod.status?.containerStatuses?.find((c) => c.state?.waiting)?.state?.waiting?.reason;
    const terminated = pod.status?.containerStatuses?.find((c) => c.state?.terminated)?.state?.terminated?.reason;
    return waiting || terminated || status;
}
function countRestarts(pod) {
...
router.get("/", (req, res) => {
    ...
    if (lower === "pod") {
        ...
        return list
            ...
            return res.json(filterResourcesByPolicy(items.map((pod) => ({
                type: "Pod",
    ...
    if (lower === "deployment") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((deploy) => ({
                type: "Deployment",
    ...
    if (lower === "daemonset") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((ds) => ({
                type: "DaemonSet",
    ...
    if (lower === "replicaset") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((rs) => ({
                type: "ReplicaSet",
    ...
    if (lower === "statefulset") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((ss) => ({
                type: "StatefulSet",
    ...
    if (lower === "job") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((job) => ({
                type: "Job",
    ...
    if (lower === "cronjob") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((cj) => ({
                type: "CronJob",
    ...
    if (lower === "service") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((svc) => ({
                type: "Service",
    ...
    if (lower === "ingress") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((ing) => ({
                type: "Ingress",
                name: ing.metadata?.name || "unknown",
                namespace: ing.metadata?.namespace || "",
                className: ing.spec?.ingressClassName || "",
                hosts: (ing.spec?.rules || []).map((rule) => rule.host).filter(Boolean).join(",")
            })), req.user));
        })
            .catch((err) => {
    ...
    if (lower === "namespace") {
        return core
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((ns) => ({
                type: "Namespace",
    ...
    if (lower === "configmap") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((cm) => ({
                type: "ConfigMap",
    ...
    if (lower === "secret") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((secret) => ({
                type: "Secret",
    ...
    if (lower === "serviceaccount") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((sa) => ({
                type: "ServiceAccount",
    ...
    if (lower === "persistentvolume") {
        return core
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((pv) => ({
    ...
    if (lower === "persistentvolumeclaim") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((pvc) => ({
    ...
    if (lower === "node") {
        return core
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((node) => ({
                type: "Node",
    ...
    if (lower === "event") {
        ...
        return list
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((event) => ({
                type: "Event",
    ...
    if (lower === "crd" || lower === "customresourcedefinition") {
        return apix
            ...
            const raw = response?.body ?? response;
            const items = raw?.items || [];
            return res.json(filterResourcesByPolicy(items.map((crd) => ({
